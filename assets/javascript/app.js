...
var i = 0;
var triviaQuestion = triviaQuestions[parseInt(i, 10)];
var time = 30;
var timer;
...
function nextQuestion() {
    ...
    i += 1;
    triviaQuestion = triviaQuestions[parseInt(i, 10)];
    time = 30;
...
function playGame() {
    ...
    if (i === triviaQuestions.length) {
        ...
        $("#totalscore")
            .css("display", "block")
            .append(`<h1>YOU SCORED: ${correctQuestions}/${numberofQuestions}</h1>`);
        if (correctQuestions <= 6) {
            var failQueryURL =
                "https://api.giphy.com/v1/gifs/random?api_key=BkaUZZWcFij6J7AoQj3WtPb1R2p9O6V9&tag=failed";
            ...
            var congratsQueryURL =
                "https://api.giphy.com/v1/gifs/random?api_key=BkaUZZWcFij6J7AoQj3WtPb1R2p9O6V9&tag=congrats";
        ...
        triviaQuestion.choices.forEach(function (choice) {
            $(
                    `<div id="${choice.charAt(0)}" class="choices"><h3>${choice}</h3></div>`
                )
                .appendTo("#choices")
                .one("click", function () {
                    if (choice.charAt(0) === triviaQuestion.answer) {
                        $(`#${triviaQuestion.answer}`).addClass("correctAnswer");
                        var correctQueryURL =
                            "https://api.giphy.com/v1/gifs/random?api_key=BkaUZZWcFij6J7AoQj3WtPb1R2p9O6V9&tag=happydance";
                        ...
                        $(`#${triviaQuestion.answer}`).addClass("correctAnswer");
                        var incorrectQueryURL =
                            "https://api.giphy.com/v1/gifs/random?api_key=BkaUZZWcFij6J7AoQj3WtPb1R2p9O6V9&tag=disappointed";
